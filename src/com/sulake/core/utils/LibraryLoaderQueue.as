package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1879:int = 4;
      ...
      {
         return this.var_456.length + this.var_297.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_297.length < const_1879 && this.var_456.length > 0)
            {
               _loc1_ = this.var_456.shift();
               this.var_297.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_456.indexOf(loader);
            if(index > -1)
            {
               this.var_456.splice(index,1);
            }
            index = this.var_297.indexOf(loader);
            if(index > -1)
            {
               this.var_297.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_297)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
