package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private var var_1587:IAvatarImage;
      ...
      private var var_412:IActiveActionData;
      ...
      private var var_2919:ByteArray;
      
      private var var_2920:ImageData;
      
      private var var_937:PaletteMap;
      
      private var var_1095:Array;
      
      private var var_1094:Point;
      ...
      public function dispose() : void
      {
         ...
         {
            _loc2_ = this.var_293.getKeys();
            for each(_loc3_ in _loc2_)
            {
               _loc1_ = this.var_293.getValue(_loc3_) as AvatarImageBodyPartCache;
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
               }
      ...
      public function disposeInactiveActions(param1:int = 60000) : void
      {
         ...
         var _loc4_:Array = this.var_293.getKeys();
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = this.var_293.getValue(_loc5_) as AvatarImageBodyPartCache;
            if(_loc3_ != null)
            {
      ...
      public function resetBodyPartCache(param1:IActiveActionData) : void
      {
         var _loc2_:* = null;
         var _loc4_:* = null;
         var _loc3_:Array = this.var_293.getKeys();
         for each(_loc4_ in _loc3_)
         {
            _loc2_ = this.var_293.getValue(_loc4_) as AvatarImageBodyPartCache;
            if(_loc2_ != null)
            {
      ...
      {
         if(this.var_896 == param1)
         {
            return;
         }
         if(this.var_896 == GeometryType.const_1261 && param1 == GeometryType.const_1252 || this.var_896 == GeometryType.const_1252 && param1 == GeometryType.const_1261)
         {
            this.var_896 = param1;
            this.var_96 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         if(this._bodyPartCache == null)
         {
            ...
            this.var_293.add(param1,this._bodyPartCache);
         }
         var _loc3_:int = this._bodyPartCache.getDirection();
         var _loc4_:int = param2;
         this.var_412 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_412;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_412 || !this.var_412.definition))
         {
            if(this.var_412.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_412.definition.state + "." + this.var_412.actionParameter);
               _loc13_ = param2 - this.var_412.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_412.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     {
                        this.var_412 = _loc14_.action;
                     }
                     if(_loc14_.type == AnimationLayerData.const_715)
                     {
                        if(_loc14_.action != null)
                        {
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1587.getFigure(),_loc5_,this.var_896,_loc3_,_loc6_,_loc7_);
         ...
         if(this.var_487 == null)
         {
            ...
            this.var_487 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_412);
            if(this.var_487 == null)
            {
               return null;
            }
      ...
      {
         var _loc2_:AvatarImageBodyPartCache = this.var_293.getValue(param1) as AvatarImageBodyPartCache;
         if(_loc2_ == null)
         {
            _loc2_ = new AvatarImageBodyPartCache();
            this.var_293.add(param1,_loc2_);
         }
         return _loc2_;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_96)
         {
            this.var_96 = this._structure.getCanvas(this._scale,this.var_896);
            if(!this.var_96)
            {
               return null;
            }
            this.var_2921 = new Point(0,this.var_96.height - int(this.var_96.width / 4));
         }
         ...
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_944 || _loc10_ == AvatarAction.const_422;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1652 = this.var_139.bodyPartId;
               this.var_203 = this.var_139.partType;
               this.var_2124 = this.var_139.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_139.flippedPartType != this.var_203)
                     {
                        this.var_203 = this.var_139.flippedPartType;
                     }
                  }
               }
               this.var_1653 = this._scale + "_" + this.var_139.action.assetPartDefinition + "_" + this.var_203 + "_" + this.var_2124 + "_" + _loc5_ + "_" + this._frame;
               this.var_488 = this._assets.getAssetByName(this.var_1653) as BitmapDataAsset;
               if(this.var_488 == null)
               {
                  this.var_1653 = this._scale + "_std_" + this.var_203 + "_" + this.var_2124 + "_" + _loc5_ + "_0";
                  this.var_488 = this._assets.getAssetByName(this.var_1653) as BitmapDataAsset;
               }
               if(!(!this.var_488 || !this.var_488.content))
               {
                  if(this.var_139.paletteMapId > -1)
                  {
                     this.var_937 = this._structure.renderManager.paletteManager.getPalette(this.var_139.paletteMapId);
                     if(this.var_937 != null)
                     {
                        this.var_1093 = this.var_488.content as BitmapData;
                        this.var_202 = new BitmapData(this.var_1093.width,this.var_1093.height);
                        this.var_2919 = this.var_937.colorize(this.var_1093.getPixels(this.var_1093.rect));
                        this.var_202.setPixels(this.var_202.rect,this.var_2919);
                     }
                     else
                     {
                        this.var_202 = (this.var_488.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1652 == AvatarBodyPartType.const_254 || this.var_1652 == AvatarBodyPartType.const_1880 || this.var_1652 == AvatarBodyPartType.const_1952) && this.var_139.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_202.colorTransform(this.var_202.rect,this.var_139.color.colorTransform);
                  }
                  if(this.var_139.isBlendable)
                  {
                     this.var_202.colorTransform(this.var_202.rect,this.var_139.blendTransform);
                  }
                  this.var_1094 = this.var_488.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_60)
                     {
                  ...
                  this.var_2920 = new ImageData(this.var_202,this.var_1094,_loc7_);
                  this.var_1095.push(this.var_2920);
               }
            }
            _loc8_--;
         }
         ...
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2921.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_60)
            {
