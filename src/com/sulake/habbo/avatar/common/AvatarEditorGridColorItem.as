package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_1006:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_1097.width,this.var_1097.height,true,0);
            _loc5_ = this.var_1097.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_1006.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_1007.width,this.var_1007.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_1007,this.var_1007.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_1006)
            {
               switch(this.var_1006.clubLevel)
               {
                  case HabboClubLevelEnum.const_56:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_36:
                     _loc4_.style = HabboIconType.const_852;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_43:
                     _loc4_.style = HabboIconType.const_889;
                     _loc4_.visible = true;
               }
            }
            else
