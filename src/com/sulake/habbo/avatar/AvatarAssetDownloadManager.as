package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2263:int = 100;
      
      private const const_1879:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_932 = new Timer(this.const_2263,1);
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_426)
         {
            this.var_426.addEventListener(AssetLoaderEvent.const_31,this.onConfigurationComplete);
            this.var_426.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_426)
         {
            this.var_426.removeEventListener(AssetLoaderEvent.const_31,this.onConfigurationComplete);
            this.var_426.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_933 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1146);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_933);
            if(this.var_1146.indexOf("?") > 0)
            {
               _loc2_ = this.var_1146 + "&retry=" + this.var_933;
            }
            else
            {
               _loc2_ = this.var_1146 + "?retry=" + this.var_933;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2295,this._assets);
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         while(_loc6_ < this.var_748.length)
         {
            _loc5_ = this.var_748[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_748.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_932.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_749.length > 0 && this.var_748.length < this.const_1879)
         {
            _loc1_ = this.var_749[0];
            _loc1_.startDownloading();
            this.var_748.push(this.var_749.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_749.indexOf(param1) == -1 && this.var_748.indexOf(param1) == -1)
         {
