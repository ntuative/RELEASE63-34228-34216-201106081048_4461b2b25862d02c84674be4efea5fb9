package com.sulake.habbo.ui.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_400,this.onQueueStatus);
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_515,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_400,this.onQueueStatus);
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_515,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_2117 = param1.type;
            this.var_1650 = false;
            this.var_1350 = param1.position;
         }
         else
         {
            this.var_1650 = true;
         }
         this.var_2916 = param1.hasHabboClub;
         this.var_2118 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1350.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1350.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1350.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1350.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_2117)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_400:
                  _loc1_.caption = !!this.var_2118 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1650;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_515:
                  _loc1_.caption = !!this.var_2118 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1650;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2916;
         }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_2117 == RoomWidgetRoomQueueUpdateEvent.const_400)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_807);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
