package com.sulake.habbo.inventory.items
{
   public class FloorItem implements IItem
   {
       
      
      protected var _id:int;
      
      protected var _ref:int;
      
      protected var _category:int;
      
      protected var _type:int;
      
      protected var var_2197:String;
      
      protected var var_1634:Number;
      
      protected var var_2977:Boolean;
      
      protected var var_2978:Boolean;
      
      protected var var_2432:Boolean;
      
      protected var var_2666:Boolean;
      
      protected var var_2993:int;
      
      protected var var_2428:int;
      
      protected var var_2431:int;
      
      protected var var_2430:int;
      
      protected var var_1921:String;
      
      protected var var_1776:int;
      
      protected var var_1110:Boolean;
      
      public function FloorItem(param1:int, param2:int, param3:int, param4:int, param5:Boolean, param6:Boolean, param7:Boolean, param8:Boolean, param9:String, param10:Number, param11:int, param12:int, param13:int, param14:int, param15:String, param16:int)
      {
         super();
         this._id = param1;
         this._type = param2;
         this._ref = param3;
         this._category = param4;
         this.var_2432 = param5;
         this.var_2978 = param6;
         this.var_2977 = param7;
         this.var_2666 = param8;
         this.var_2197 = param9;
         this.var_1634 = param10;
         this.var_2993 = param11;
         this.var_2428 = param12;
         this.var_2431 = param13;
         this.var_2430 = param14;
         this.var_1921 = param15;
         this.var_1776 = param16;
      }
      
      public function get id() : int
      {
         return this._id;
      }
      
      public function get ref() : int
      {
         return this._ref;
      }
      
      public function get category() : int
      {
         return this._category;
      }
      
      public function get type() : int
      {
         return this._type;
      }
      
      public function get stuffData() : String
      {
         return this.var_2197;
      }
      
      public function get extra() : Number
      {
         return this.var_1634;
      }
      
      public function get recyclable() : Boolean
      {
         return this.var_2977;
      }
      
      public function get tradeable() : Boolean
      {
         return this.var_2978;
      }
      
      public function get groupable() : Boolean
      {
         return this.var_2432;
      }
      
      public function get sellable() : Boolean
      {
         return this.var_2666;
      }
      
      public function get expires() : int
      {
         return this.var_2993;
      }
      
      public function get creationDay() : int
      {
         return this.var_2428;
      }
      
      public function get creationMonth() : int
      {
         return this.var_2431;
      }
      
      public function get creationYear() : int
      {
         return this.var_2430;
      }
      
      public function get slotId() : String
      {
         return this.var_1921;
      }
      
      public function get songId() : int
      {
         return this.var_1776;
      }
      
      public function set locked(param1:Boolean) : void
      {
         this.var_1110 = param1;
      }
      
      public function get locked() : Boolean
      {
         return this.var_1110;
      }
   }
}
