package com.sulake.habbo.room.utils
{
   import com.sulake.core.utils.Map;
   
   public class RoomInstanceData
   {
       
      
      private var _roomId:int = 0;
      
      private var _roomCategory:int = 0;
      
      private var var_862:TileHeightMap = null;
      
      private var var_1322:LegacyWallGeometry = null;
      
      private var var_1321:RoomCamera = null;
      
      private var var_864:SelectedRoomObjectData = null;
      
      private var var_863:SelectedRoomObjectData = null;
      
      private var _worldType:String = null;
      
      private var var_570:Map;
      
      private var var_571:Map;
      
      public function RoomInstanceData(param1:int, param2:int)
      {
         this.var_570 = new Map();
         this.var_571 = new Map();
         super();
         this._roomId = param1;
         this._roomCategory = param2;
         this.var_1322 = new LegacyWallGeometry();
         this.var_1321 = new RoomCamera();
      }
      
      public function get roomId() : int
      {
         return this._roomId;
      }
      
      public function get roomCategory() : int
      {
         return this._roomCategory;
      }
      
      public function get tileHeightMap() : TileHeightMap
      {
         return this.var_862;
      }
      
      public function set tileHeightMap(param1:TileHeightMap) : void
      {
         if(this.var_862 != null)
         {
            this.var_862.dispose();
         }
         this.var_862 = param1;
      }
      
      public function get legacyGeometry() : LegacyWallGeometry
      {
         return this.var_1322;
      }
      
      public function get roomCamera() : RoomCamera
      {
         return this.var_1321;
      }
      
      public function get worldType() : String
      {
         return this._worldType;
      }
      
      public function set worldType(param1:String) : void
      {
         this._worldType = param1;
      }
      
      public function get selectedObject() : SelectedRoomObjectData
      {
         return this.var_864;
      }
      
      public function set selectedObject(param1:SelectedRoomObjectData) : void
      {
         if(this.var_864 != null)
         {
            this.var_864.dispose();
         }
         this.var_864 = param1;
      }
      
      public function get placedObject() : SelectedRoomObjectData
      {
         return this.var_863;
      }
      
      public function set placedObject(param1:SelectedRoomObjectData) : void
      {
         if(this.var_863 != null)
         {
            this.var_863.dispose();
         }
         this.var_863 = param1;
      }
      
      public function dispose() : void
      {
         if(this.var_862 != null)
         {
            this.var_862.dispose();
            this.var_862 = null;
         }
         if(this.var_1322 != null)
         {
            this.var_1322.dispose();
            this.var_1322 = null;
         }
         if(this.var_1321 != null)
         {
            this.var_1321.dispose();
            this.var_1321 = null;
         }
         if(this.var_864 != null)
         {
            this.var_864.dispose();
            this.var_864 = null;
         }
         if(this.var_863 != null)
         {
            this.var_863.dispose();
            this.var_863 = null;
         }
         if(this.var_570 != null)
         {
            this.var_570.dispose();
            this.var_570 = null;
         }
         if(this.var_571 != null)
         {
            this.var_571.dispose();
            this.var_571 = null;
         }
      }
      
      public function addFurnitureData(param1:FurnitureData) : void
      {
         if(param1 != null)
         {
            this.var_570.remove(param1.id);
            this.var_570.add(param1.id,param1);
         }
      }
      
      public function getFurnitureData() : FurnitureData
      {
         if(this.var_570.length > 0)
         {
            return this.getFurnitureDataWithId(this.var_570.getKey(0));
         }
         return null;
      }
      
      public function getFurnitureDataWithId(param1:int) : FurnitureData
      {
         return this.var_570.remove(param1);
      }
      
      public function addWallItemData(param1:FurnitureData) : void
      {
         if(param1 != null)
         {
            this.var_571.remove(param1.id);
            this.var_571.add(param1.id,param1);
         }
      }
      
      public function getWallItemData() : FurnitureData
      {
         if(this.var_571.length > 0)
         {
            return this.getWallItemDataWithId(this.var_571.getKey(0));
         }
         return null;
      }
      
      public function getWallItemDataWithId(param1:int) : FurnitureData
      {
         return this.var_571.remove(param1);
      }
   }
}
