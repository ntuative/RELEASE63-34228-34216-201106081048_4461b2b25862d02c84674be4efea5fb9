package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_469.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_469.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_469.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_469.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_469.view.setNewMessageArrived();
                  this.var_469.notifications.method_13(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_1269).received = true;
